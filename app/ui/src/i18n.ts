...
const D: Record<string, [string, string, string]> = {
  ...
  "f.hit": ["缓存命中", "cache hit", "Cache-Treffer"],
  ...
  "f.staleToken": ["凭证已过期", "login token expired", "Token abgelaufen"],
  "f.loginRequired": ["Claude Code 登录已失效，需重新登录", "Claude Code login expired — sign in again", "Claude-Code-Login abgelaufen — neu anmelden"],
  "f.loginExpiring": ["Claude Code 登录 {h} 后到期", "Claude Code login expires in {h}", "Claude-Code-Login läuft in {h} ab"],
  "f.goLogin": ["去登录", "Sign in", "Anmelden"],
  "f.goLoginTip": ["打开终端运行 claude auth login，会弹出浏览器授权；登录完成后这里会自动恢复", "Opens a terminal running claude auth login (browser sign-in); this recovers automatically once done", "Öffnet ein Terminal mit claude auth login (Browser-Anmeldung); danach erholt sich die Anzeige automatisch"],
  "f.refreshLogin": ["重新登录", "Re-login", "Neu anmelden"],
  "f.refreshing": ["登录中…", "Logging in…", "Anmeldung…"],
  "f.refreshOk": ["已恢复", "Fixed", "Behoben"],
  "f.refreshFail": ["失败：{e}", "Failed: {e}", "Fehlgeschlagen: {e}"],
